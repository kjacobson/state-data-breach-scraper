...
(async () => {
  ...
  for (let i = years.length - 1; i >= 0; i--) {
      const currentYearLink = await page.waitForSelector(`#navigation > ul > li:nth-child(8) > ul > li:nth-child(${i + 1}) > a[href]`);
      const nav = page.waitForNavigation();
      await page.evaluate((el) => el.click(), currentYearLink);
      await nav;

      const table = await page.waitForSelector('#content table[role="presentation"]');
      await page.waitForFunction(() => (
          document.querySelectorAll('#content table[role="presentation"] > tbody > tr').length > 0
      ));
