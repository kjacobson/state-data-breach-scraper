...
(async () => {
  ...
  await page.goto('https://www.atg.wa.gov/data-breach-notifications', { waitUntil: 'networkidle0' });
  ...
  let done = false;
  while (!done) {
      const selectedPageLI = await page.waitForSelector('.pagination > li.active > a');
      ...
      const table = await page.waitForSelector('#block-views-data-breach-notifications-block .view-content .view-table');
      await page.waitForFunction(() => (
          document.querySelectorAll('#block-views-data-breach-notifications-block .view-content .view-table > tbody > tr').length > 0
      ));
      const rows = await table.$$('tbody > tr');
      for (const row of rows) {
        try {
            const [reported, bizName, start, affected, dataTypes] = await row.$$("td");
            ...
            let dataAccessed = await page.evaluate((el) => el.textContent, dataTypes);
      ...
      if (selectedPageNumber !== "1") {
          const prevPageLink = await page.waitForSelector('.prev.page-numbers a[href]');
