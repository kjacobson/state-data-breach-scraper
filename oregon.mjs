...
export const handler = async () => {
  ...
  while (!err) {
    ...
    try {
      const currentPage = await page.waitForSelector(
        '#grid_paginate .paginate_button.current'
      )
      ...
      const nav = page.waitForFunction(
        (currentPageText) => {
  ...
  await page.waitForSelector(
    '#grid_paginate .paginate_button.current:last-child'
  )
    ...
    for (const row of rows) {
      try {
        ...
        if (dateString.trim().indexOf(',') > -1) {
          ...
          breachDates = breachDates.map((str) =>
            str
              .split('-')
              .map((date) => date.trim())
              .join(' - ')
