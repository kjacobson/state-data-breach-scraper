...
(async () => {
  ...
  await page.goto('https://oag.my.site.com/datasecuritybreachreport/apex/DataSecurityReportsPage', { waitUntil: 'networkidle0' });
  ...
  let done = false;
  while (!done) {
      const selectedPageLI = await page.waitForSelector('.paginate_button.current');
      ...
      for (const row of rows) {
        try {
            ...
            const businessState = await page.evaluate((el) => el.textContent.trim(), state);
            const businessZip = await page.evaluate((el) => el.textContent.trim(), zip);
            const publishedDate = await page.evaluate((el) => el.textContent.trim(), published);
