...
(async () => {
  ...
  const lastPage = await page.waitForSelector('#DataTables_Table_0_paginate > span > a.paginate_button:last-child');
  await page.evaluate((el) => el.click(), lastPage);
  await page.waitForSelector('#DataTables_Table_0_next.disabled');

  let done = false;
  while (!done) {
      const selectedPageLink = await page.waitForSelector('.paginate_button.current');
      const selectedPageNumber = await page.evaluate((el) => el.textContent, selectedPageLink);
      ...
      if (selectedPageNumber !== "1") {
          const prevPageLink = await page.evaluateHandle(el => el.previousElementSibling, selectedPageLink);
