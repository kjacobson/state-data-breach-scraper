...
(async () => {
    ...
    for (let i = 0; i < years.length; i++) {
        ...
        while (!done) {
            years = await page.$$('#onetidDoclibViewTbl0 > tbody[id^="titl"]');
            year = years[i];
            const dataTableBody = await page.evaluateHandle(el => el.nextElementSibling, year);
            await page.waitForFunction((i) => {
                const yearTBodies = document.querySelectorAll('#onetidDoclibViewTbl0 > tbody[id^="titl"]');
                const yearTBody = yearTBodies[i];
                const dataTBody = yearTBody.nextElementSibling;
                const rows = dataTBody.childNodes;
                return rows.length > 1;
            }, {}, i);
            const paginationTableBody = await page.evaluateHandle(el => el.nextElementSibling, dataTableBody);
            const rows = await dataTableBody.$$('tr');
            for (const row of rows) {
                try {
                    const [bizName, _, reported, affected, data, type] = await row.$$("td[role='gridcell']");
            ...
            try {
                const pageRange = await paginationTableBody.waitForSelector('.ms-paging');
                const pageRangeText = await page.evaluate(el => el.textContent, pageRange);
                const nextPageLink = await paginationTableBody.waitForSelector('.ms-promlink-button-enabled[title="Next"]');
                const loading = page.waitForFunction((i, pageRangeText) => {
                    const yearTBodies = document.querySelectorAll('#onetidDoclibViewTbl0 > tbody[id^="titl"]');
                    const yearTBody = yearTBodies[i];
                    const dataTBody = yearTBody.nextElementSibling;
                    const pagingTBody = dataTBody.nextElementSibling;
                    const newPageRange = pagingTBody.getElementsByClassName('ms-paging')[0].textContent;
